...
const AvatarGroup = ({
  ...
  return (
    <View
      style={[
        containerStyle,
        { overflow: 'hidden', width: size, height: size, borderRadius: size },
      ]}
    >
      {renderAvatars()}
    </View>
...
const getHeightPoint = (_: number, total: number): number => {
  if (total === 2) {
    return 1
  }
  return 0.5
}
const getWidthPoint = (idx: number, total: number): number => {
  if (total === 3 && idx === 0) {
    return 1
  }
  return 0.5
}
const getTopPoint = (idx: number, total: number): number => {
  if (total === 2) {
    return -0.025
  }
  if (total === 3 && idx === 0) {
...
const getLeftPoint = (idx: number): number => {
  if (idx === 0 || idx === 2) {
    return -0.025
  }
  return 0.525
}
const getInnerLeft = (idx: number, total: number): number => {
  if (total === 3 && idx === 0) {
    return 0
  }
  return 0.25
}
const getInnerTop = (total: number): number => {
  if (total === 2) {
    return 0
  }
  return 0.25
