...
const SettingsScreen = (): ReactElement => {
  ...
  const onChangeProfileImage = (): void => {
    openMenu({
  ...
  const onToggleDisturb = async (): Promise<void> => {
  ...
  const onExitToHome = async (): Promise<void> => {
  ...
  const menuItems: MenuBarProps[] = [
    {
      ...
      actionItem: (
        <Switch value={scheme === 'dark'} onChangeValue={onToggleTheme} />
      ),
    },
    {
      ...
      actionItem: (
        <Switch value={option === 'off'} onChangeValue={onToggleDisturb} />
      ),
    },
    {
      icon: 'leave',
